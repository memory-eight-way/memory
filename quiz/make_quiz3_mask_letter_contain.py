...
def proc_line_word_mask(line,lv):
    ...
    for w_word in w_words:
        if chk_char not in w_word.lower():
...
def write_quiz_file(li_quiz,lv,w_chap_file_name,w_quiz_dir_suffix=""):    
    ...
    if FLAG_ANSWER:
        fn_quiz="./quiz3-a"+w_quiz_dir_suffix+"/lv"+slv+"_"+f_suffix+"/"+w_chap_file_name
    else:
        fn_quiz="./quiz3/"+w_quiz_dir_suffix+"/lv"+slv+"_"+f_suffix+"/"+w_chap_file_name
...
def make_quiz_line(lines,lv):
    ...
    return ("mask_word_"+chk_char+"_contain",proc_txt_mask_word(lines,lv))
